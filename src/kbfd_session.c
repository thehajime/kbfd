...
bfd_session_new(struct bfd_proto *proto, struct sockaddr *dst, int ifindex)
{
	...
	if (bfd){
		...
		BFD_INIT_WORK(&bfd->t_tx_work, bfd_xmit_timeout, bfd);
		BFD_INIT_WORK(&bfd->t_rx_expire, bfd_detect_timeout, bfd);

		bfd->dst = bfd_malloc(bfd->proto->namelen(dst));
		if (!bfd->dst){
			bfd_free(bfd);
			return NULL;
		}
		...
		bfd->src = bfd_malloc(bfd->proto->namelen(dst));
		if (!bfd->src){
			bfd_free(bfd->dst);
			bfd_free(bfd);
			return NULL;
...
bfd_session_add(struct bfd_proto *proto, struct sockaddr *dst, int ifindex)
{
	...
	if (IS_DEBUG_BSM){
		blog_info("session %s, disc=%d add",
				  proto->addr_print(bfd->dst, buf),
				  bfd->cpkt.my_disc);
	}
...
bfd_start_xmit_timer(struct bfd_session *bfd)
{
	...
	bfd_workqueue_add(master->tx_ctrl_wq, &bfd->t_tx_work,
	    bfd->act_tx_intv, jitter);
	return;
}
...
{
	bfd_workqueue_delete(master->tx_ctrl_wq, &bfd->t_tx_work);
	return;
}
...
{
	bfd_workqueue_delete(master->ctrl_expire_wq, 
	    &bfd->t_rx_expire);
	return;
}
...
bfd_reset_expire_timer(struct bfd_session *bfd)
{
	...
	bfd_workqueue_add(master->ctrl_expire_wq, &bfd->t_rx_expire,
	    bfd->detect_time, 100);
	return;
}
...
bfd_session_init(void)
{
	...
	master->tx_ctrl_wq = bfd_workqueue_init("kbfd_tx");
	if (!master->tx_ctrl_wq){
		blog_err("failed create workqueue");
	}

	/* Workqueue for receive expire */
	master->ctrl_expire_wq = bfd_workqueue_init("kbfd_rx_expire");
...
bfd_session_finish(void)
{
	...
	if (master->ctrl_expire_wq){
		bfd_workqueue_exit(master->ctrl_expire_wq);
		master->ctrl_expire_wq = NULL;
	}

	if (master->tx_ctrl_wq){
		bfd_workqueue_exit(master->tx_ctrl_wq);
